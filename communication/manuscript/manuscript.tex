...
conditional rule reduced true reliability in 46.7\% of the replications
in which it acted, despite acting only when in-sample alpha appeared to
improve, and the forced rule did so in 49.6\% of all replications.
Individual studies encounter outcomes far more extreme than the average
bias suggests. A post hoc analysis of stricter decision rules shows that
thresholding the apparent improvement helps only when samples are large:
requiring an apparent gain of at least .02 with N of 200 or more kept
the probability that a drop harms true reliability at or below 7\%
wherever the rule recommended dropping at all, whereas at N = 20 even
requiring a gain of .05 left that probability near 38\% for five-item
scales. As a rule of thumb, an alpha-if-item-removed recommendation
should not be acted on in small samples at all, and in larger samples
only when the apparent improvement exceeds .02. Because this risk
depends only on scale length and sample size, given a loading
distribution typical of trait scales, it is knowable before data are
