...
if(file.exists("../../data/intermediary/data_replications.rds")){
  ...
  data_nested_single_timepoint_25 <- data_nested_single_timepoint |>
    mutate(replications = furrr::future_map(data, generate_replications, n_replications = 1000, subset_n_per_split = 25),
           subset_n_per_split = 25) |>
    select(-data) |>
    unnest(replications)
  
  data_nested_single_timepoint_50 <- data_nested_single_timepoint |>
    mutate(replications = furrr::future_map(data, generate_replications, n_replications = 1000, subset_n_per_split = 50),
           subset_n_per_split = 50) |>
    select(-data) |>
    unnest(replications)
  
  data_nested_single_timepoint_100 <- data_nested_single_timepoint |>
    mutate(replications = furrr::future_map(data, generate_replications, n_replications = 1000, subset_n_per_split = 100),
           subset_n_per_split = 100) |>
    select(-data) |>
    unnest(replications)
  
  data_nested_single_timepoint_250 <- data_nested_single_timepoint |>
    mutate(replications = furrr::future_map(data, generate_replications, n_replications = 1000, subset_n_per_split = 250),
           subset_n_per_split = 250) |>
    select(-data) |>
    unnest(replications)
  
  data_nested_single_timepoint_500 <- data_nested_single_timepoint |>
    mutate(replications = furrr::future_map(data, generate_replications, n_replications = 1000, subset_n_per_split = 500),
...
# Calcualte alpha and drop decisions for each sample
