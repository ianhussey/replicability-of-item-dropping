...
- move from replication to novel study. drop the EFA replication piece? not if Leo is running with it as a thesis.
- add conditional dropping so that item(s) are only dropped if alpha improves, or better yet if it crosses a .6, .7, .8, .9 threshold. might need to check which of K multiple number of drops is the best and use that, or the first that reaches a threshold.
- Find one or more more realistic distributions of alpha using real datasets, e.g., synth training data, bainbridge, AIID.
- Choose a strategy that purposefully adds a poorly performing item to the realistic distribution. Compare via sim the probability of choosing to drop an item when there is vs isnt a bad one, and the probability of choosing the wrong item to drop when there is a bad one.

# Overview 

TODO

# Simulation conditions used by Kopalle & Lehman (1997)

Although this simulation differs from that by Kopalle & Lehman (1997) in important ways: 

1. Kopalle & Lehman (1997) employed only 50 iterations whereas I will employ several thousand. This is presumably due to how manual their method was or the processing power available at the time. 
2. Kopalle & Lehman (1997) dropped items by factor analyzing the items and choosing the *k* items that loaded most strongly onto the first factor). These simulations drop items based on both K&L97's method but also by dropping items based on max-Cronbach's-alpha-if-item-removed.

These simulations aim to be similar to Kopalle and Lehman (1997) its data generation processes. The data generation processes were described in Kopalle & Lehman (1997) as follows:
...
\TODO: extension of this simulation to consider matched pairs of sims, or proportion agreement across iterations of which items are to be retained.
...
generate_model_uniform_distribution <- function(k_indicators, lambda_mu) {
  ...
  # generate each item string
  itemStrings <- sapply(1:k_indicators, function(i) {
    paste(format(runif(n = 1, min = lambda_mu - 0.1, max = lambda_mu + 0.1), nsmall = 2), "*item", i, sep = "") # range [lambda_mu 0.1, lambda_mu+0.1)] produces lambda_sigma = 0.58; same as used in other distributions. see Kopalle & Lehman (1997).
  })
...
#   iteration = 1:3 # Kopalle & Lehman (1997) used 50
# )
...
ggplot(simulation_bias, aes(alpha + bias, # sub set alpha is full + bias
                            bias*-1, 
                            #group = n_participants,
                            #group = lambda_distribution,
                            shape = lambda_distribution,
